#!/usr/bin/env node
// cloudflare-workers-mcp · stdio MCP server exposing foldkit as Claude-callable tools + resources
...
const server = new Server(
  { name: 'cloudflare-workers-mcp', version: '1.0.0' },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `cloudflare-workers-mcp error: ${err.message}` }]
    };
  }
});
